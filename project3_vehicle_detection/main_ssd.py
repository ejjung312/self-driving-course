...
from project3_vehicle_detection.SSD import process_frame_bgr_with_SSD, get_SSD_model
from project3_vehicle_detection.vehicle import Vehicle
...
def process_pipeline(frame, verbose=False):
    ...
    cv2.putText(img_blend_out, 'Vehicles in sight: {:02d}'.format(len(detected_vehicles)), (20, off_y + thumb_h // 2), font, 0.8, (255, 255, 255), 2, cv2.LINE_AA)
...
if __name__ == '__main__':
    ...
    if mode == 'video':
        video_file = 'project2_advanced_lane_detection_v2/test_videos/project_video.mp4'

        cap_in = cv2.VideoCapture(video_file)
        video_out_dir = 'project3_vehicle_detection/frames_out'
